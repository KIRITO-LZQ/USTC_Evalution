...
$(document).ready(function (){
    ...
    $(".group-items").on("click",function (){
        ...
        // curChatUserNum = this.getAttribute("data-groupId");
        ...
        //消息先存进localStorage,通过localStorage导入聊天窗口
        sessToLocalStorage(curChatUserNum);
        let json={"groupNum":curChatUserNum};
        $(".message-item-box").html(localStorage.getItem(curChatUserNum));
        sendReadConfirm();
        $.ajax({
            type: "POST",
            ...
            var num_of_stu=0;
            var variance=0;
            num_of_stu=res['one']+res['two']+res['three']+res['four']+res['five']+res['six'];
            avg=(res['one']+res['two']*2+res['three']*3+res['four']*4+res['five']*5+res['six']*6)/num_of_stu;
            variance=(Math.pow(1-avg,2)*res['one']+Math.pow(2-avg,2)*res['two']+Math.pow(3-avg,2)*res['three']+Math.pow(4-avg,2)*res['four']+Math.pow(5-avg,2)*res['five']+Math.pow(6-avg,2)*res['six'])/num_of_stu;
                myChart.setOption({
                    series: [{
                        // 根据名字对应到相应的系列
                        name: '各表情人数',
    ...
    $("#send_score").on("click",function(){
        let preMsg = $("#score:checked").val();
		console.log(preMsg);
        // $("#pre-send-txt").val('');
        if (!preMsg.match(/^\s+$/) && preMsg!==''){
                let json = {"msgTypeCode":1,"toUserNum":curChatUserNum,"message":preMsg,"sendTime":new Date().getTime()};
                // console.log(json);
                ws.send(JSON.stringify(json));
